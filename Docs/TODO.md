...
### Executive Planning
- [ ] **Calendar Guard:** Engineer a custom script that merges GOG calendar read-access with the `nxt-pulse-agent` physiological semantic thresholds to protect the schedule dynamically.

### Human Optimization
- [ ] **Flow State Monitoring:** Build a custom logic layer to analyze `CatchMe` or `activity-analyzer` application-switching frequencies. This folds OS state into the prompt to infer focus states and trigger notifications.
- [ ] **Micro-Suck Generation:** Write a custom `SKILL.md` that utilizes a randomized task matrix, an LLM query, and standard output messaging to issue minor resilience challenges.
