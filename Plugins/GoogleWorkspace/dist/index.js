 ...
 */
async function summarizeWithSubAgent(api, data, actionContext, model) {
 ...
 */
async function executeWorkspaceAction(api, composioApiKey, composioAction, args, model, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt === maxRetries) {
                return { success: false, error: `Failed after ${maxRetries} retries: ${e.message}. Please correct and retry.` };
            }
...
function register(api, config) {
    ...
    const registerWorkspaceTool = (name, description, composioCmd) => {
        api.registerTool({
            name,
            description,
            execute: async (args) => {
