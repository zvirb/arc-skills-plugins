...
def execute_composite(arguments_json_str, max_retries=3):
    ...
    print(f"Step 1: Fetching raw data for Google Drive Research Files...")
    raw_data = None
    try:
    ...
    print(f"Step 2: Processing data through OpenClaw skills (tavily,summarize)...")
    for attempt in range(1, max_retries + 1):
        try:
            ...
            prompt = f"Analyze and research these files using Tavily if external context is needed: {safe_data}"
            
            infer_cmd = ["wsl", "--", "bash", "-c", f"openclaw infer '{prompt}' --skills tavily,summarize"]
