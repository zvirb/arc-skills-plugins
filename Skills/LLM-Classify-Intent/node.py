...
def execute_node(arguments_json_str, max_retries=3):
    ...
    prompt = f"Classify the following text as either 'actionable' or 'informational'. Also determine 'urgency' (high, medium, low). Output ONLY valid JSON: {{'intent': 'actionable', 'urgency': 'high', 'reasoning': '...'}}. Text: {text}"

    for attempt in range(1, max_retries + 1):
        print(f"Attempt {attempt}: Executing LLM Classify Intent transformation...")
        try:
