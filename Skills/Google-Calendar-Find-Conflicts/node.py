...
def execute_composite(arguments_json_str, max_retries=3):
    ...
    print(f"Step 1: Fetching raw data for Google Calendar Find Conflicts...")
    raw_data = None
    try:
        cli_args = " ".join([f"--{k}='{v}'" for k, v in arguments.items()])
        cmd = ["wsl", "--", "bash", "-c", f"gog calendar list {cli_args} --json"]
        ...
        if Composio:
            ...
            res = client.tools.execute("GOOGLECALENDAR_FIND_EVENT", arguments=arguments)
            if res.successful:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            prompt = f"Analyze these calendar events and identify any overlapping times or conflicts: {safe_data}"
