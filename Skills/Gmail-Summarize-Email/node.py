...
def execute_composite(arguments_json_str, max_retries=3):
    ...
    print(f"Step 1: Fetching raw data for Gmail Summarize Email...")
    raw_data = None
    try:
        cli_args = " ".join([f"--{k}='{v}'" for k, v in arguments.items()])
        cmd = ["wsl", "--", "bash", "-c", f"gog gmail get {cli_args} --json"]
        ...
        if Composio:
            ...
            res = client.tools.execute("GMAIL_GET_MESSAGE", arguments=arguments)
            if res.successful:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            prompt = f"Summarize this email thoroughly: {safe_data}"
