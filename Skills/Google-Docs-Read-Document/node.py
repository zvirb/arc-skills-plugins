...
def execute_node(arguments_json_str, max_retries=3):
    ...
    for attempt in range(1, max_retries + 1):
        print(f"Attempt {attempt}: Executing Google Docs Read Document...")
        
        # Try native gog
        try:
            # Flatten args for basic CLI
            cli_args = " ".join([f"--{k}='{v}'" for k, v in arguments.items()])
            cmd = ["wsl", "--", "bash", "-c", f"gog docs read {cli_args} --json"]
        ...
        try:
            if Composio:
                ...
                res = client.tools.execute("GOOGLEDOCS_GET_DOCUMENT", arguments=arguments)
