...
def execute_cross_service(arguments_json_str):
    ...
    prompt = f"""Search emails for invoices or receipts. Extract the Vendor, Date, and Amount. Append these as a new row into the specified Google Sheet.
...
"""
    print(f"Executing Cross-Service Workflow: Workspace Invoice Extraction...")
    
    # We rely on OpenClaw's autonomous loop to handle the tool orchestration
    cmd = ["wsl", "--", "bash", "-c", f"openclaw infer '{prompt}' --skills Gmail-Search-Emails,Gmail-Retrieve-Email,Google-Sheets-Append-Row,summarize"]
