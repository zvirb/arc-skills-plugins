...
def execute_cross_service(arguments_json_str):
    ...
    prompt = f"""Check the calendar for conflicting events today. For any overlaps, find free slots later in the week, and draft an email to the attendees proposing the new times.
...
"""
    print(f"Executing Cross-Service Workflow: Workspace Proactive Rescheduler...")
    
    # We rely on OpenClaw's autonomous loop to handle the tool orchestration
    cmd = ["wsl", "--", "bash", "-c", f"openclaw infer '{prompt}' --skills Google-Calendar-Find-Event,Gmail-Draft-Email,summarize"]
