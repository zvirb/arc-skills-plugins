...
def execute_composite(arguments_json_str, max_retries=3):
    ...
    print(f"Step 1: Fetching raw data for Google Contacts Find Duplicates...")
    raw_data = None
    try:
        cli_args = " ".join([f"--{k}='{v}'" for k, v in arguments.items()])
        cmd = ["wsl", "--", "bash", "-c", f"gog contacts search {cli_args} --json"]
        ...
        if Composio:
            ...
            res = client.tools.execute("GOOGLECONTACTS_SEARCH_CONTACTS", arguments=arguments)
            if res.successful:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            prompt = f"Identify duplicate contacts in this list based on similar names or emails: {safe_data}"
